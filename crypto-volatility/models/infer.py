    ...
    def predict(self, features: pd.DataFrame) -> np.ndarray:
                ...
                if hasattr(self.scaler, "feature_names_in_"):
                    ...
                    if len(available_scaler_features) == expected_n_features:
                        ...
                        X = (
                            ...
                            else X.iloc[:, :expected_n_features]
                        )
                        logger.warning(f"Using scaler's feature order: {list(X.columns)}")
                else:
                    # Fallback: use first N features that scaler expects
                    logger.warning(
                        f"Scaler expects {expected_n_features} features but got {X.shape[1]}. Using first {expected_n_features} features."
                    )
        ...
        if hasattr(self.model, "n_features_in_"):
            ...
            if X_scaled.shape[1] != model_n_features:
                logger.warning(
                    f"Model expects {model_n_features} features but got {X_scaled.shape[1]}. Adjusting."
                )
                if X_scaled.shape[1] > model_n_features:
        ...
        if self.scaler is not None and scaler_features:
            ...
            if model_expected_features:
                ...
                available_model_features = [
                    f for f in model_expected_features if f in X_scaled_df.columns
                ]
                ...
                if len(available_model_features) == len(model_expected_features):
                    ...
                    logger.debug(
                        f"Selected {len(model_expected_features)} model features: {model_expected_features}"
                    )
                else:
                    # Some model features missing - log what we have vs what we need
                    missing_features = [
                        f for f in model_expected_features if f not in X_scaled_df.columns
                    ]
                    logger.warning(
                        f"Model expects {len(model_expected_features)} features: {model_expected_features}"
                    )
                    logger.warning(f"Available in scaled features: {list(X_scaled_df.columns)}")
                    logger.warning(f"Missing features: {missing_features}")
                    logger.warning(
                        f"Found {len(available_model_features)}/{len(model_expected_features)} model features"
                    )
...
def benchmark_inference(
    predictor: VolatilityPredictor, test_features: pd.DataFrame, n_iterations: int = 100
):
...
def main():
    ...
    parser.add_argument(
        "--model-type", type=str, default="ml", choices=["ml", "baseline"], help="Model type"
    )
    parser.add_argument(
        "--features-file",
        type=str,
    ...
    try:
            ...
            if experiment:
                ...
                if not runs.empty:
                    ...
                    if "val_best_threshold" in run.data.metrics:
                        ...
                        logger.info(
                            f"Loaded optimal threshold from MLflow: {optimal_threshold:.4f}"
                        )
        ...
        if optimal_threshold is not None:
            ...
            logger.warning(
                f"Using {threshold_method} threshold: {threshold:.4f} (consider training with threshold tuning)"
            )

        logger.info(f"Using {threshold_method} threshold: {threshold:.4f}")
        logger.info(
            f"Probability statistics: min={prob_values.min():.4f}, max={prob_values.max():.4f}, "
        ...
        logger.info(
            f"Predicted {spike_count} volatility spikes ({spike_count/len(predictions)*100:.2f}%)"
        )
