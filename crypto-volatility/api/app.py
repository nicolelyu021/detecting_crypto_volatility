...
PREDICTION_COUNTER = Counter(
    "volatility_predictions_total", "Total number of predictions made", ["prediction"]
)
...
def load_model(config_path: str = "config.yaml"):
        ...
        if mlflow_model is None:
            ...
            try:
                ...
                if len(runs) > 0:
                    ...
                    logger.info(
                        f"✅ Loaded model from MLflow run: {run_id} (name: {model_run_name})"
                    )
                else:
                    logger.warning(
                        f"No runs found with name '{model_run_name}' in experiment '{experiment_name}'"
                    )
            except Exception as e:
    ...
    logger.info(
        f"✅ Model loaded successfully from {loaded_from} (version: {model_version}) in {load_time:.3f}s"
    )
