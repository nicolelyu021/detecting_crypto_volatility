...
class KafkaReplay:
    ...
    def _extract_10_minutes(self, ticks: List[Dict], start_time: float = None) -> List[Dict]:
        ...
        filtered_ticks = [
            tick for tick in ticks_sorted
            if start_time <= tick.get('ingestion_timestamp', 0) <= end_time
        ]
        
        logger.info(f"Extracted {len(filtered_ticks)} ticks from 10-minute window "
    ...
    def replay(self, data_file: str = None, duration_minutes: int = 10, speedup: float = 1.0):
        ...
        logger.info(f"Replaying {len(filtered_ticks)} ticks over {duration_minutes} minutes "
                   f"(speedup: {speedup}x)")
        ...
        for i, tick in enumerate(filtered_ticks):
            ...
            if (i + 1) % 100 == 0:
                ...
                logger.info(f"Published {i+1}/{len(filtered_ticks)} ticks "
...
def main():
    ...
    parser.add_argument(
        ...
        type=int,
        default=10,
        help='Duration to replay in minutes (default: 10)'
    )
    parser.add_argument(
        '--speedup',
        type=float,
