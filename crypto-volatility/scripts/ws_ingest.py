...
class CoinbaseWSIngestor:
            ...
            if "channel" in data:
                ...
                if data["type"] == "subscriptions":
                    ...
                    if "channel" in error_msg.lower() or "unmarshal" in error_msg.lower():
                        logger.error(
                            "Subscription format error detected. This won't be fixed by reconnecting."
                        )
                        if self.subscription_error_count >= 3:
                            logger.error(
                                "Too many subscription errors. Exiting to prevent infinite loop."
                            )
                            self.running = False
                            if self.ws:
                                self.ws.close()
    ...
    def _check_heartbeat(self):
        ...
        if time_since_heartbeat > heartbeat_interval * 2:
            logger.warning(
                f"No heartbeat received for {time_since_heartbeat:.1f} seconds. Reconnecting..."
            )
            self.ws.close()
